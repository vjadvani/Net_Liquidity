...
function get-recent-tga ()
{
    ...
    $result = Invoke-RestMethod -Method Get -Uri ('https://api.fiscaldata.treasury.gov/services/api/fiscal_service/v1/accounting/dts/operating_cash_balance?filter=record_date:gte:{0},account_type:eq:Treasury General Account (TGA) Closing Balance&fields=record_date,open_today_bal&page[number]=1&page[size]=300' -f $date)
...
function get-recent-wlad ()
{
    $result = Invoke-RestMethod 'https://fred.stlouisfed.org/graph/fredgraph.csv?id=WLAD'
...
$ol_dates = $ol                   | ForEach-Object { $_.DATE }
...
foreach ($elt in $table | Select-Object -Skip 1)
{
    ...
    $fed_change = $elt.fed           - $prev.fed;            $fed_color = if ($fed_change -gt 0) { 'Green' } elseif ($fed_change -lt 0) { 'Red'   } else { 'White' }
    $ol_change  = $elt.ol            - $prev.ol;             $ol_color = if ($ol_change -gt 0)  { 'Green' } elseif  ($ol_change -lt 0) { 'Red'   } else { 'White' }
    ...
    Write-Host ('{0,20}' -f $elt.ol.ToString('N0'))           -NoNewline; Write-Host ('{0,20}' -f $ol_change.ToString('N0')) -ForegroundColor $ol_color -NoNewline
...
Write-Host 'DATE                      WALCL              CHANGE                 RRP              CHANGE                 TGA              CHANGE                  OL              CHANGE       NET LIQUIDITY              CHANGE    SPX FV'
    ...
    chart = @{
        ...
        data = @{
            ...
            datasets = @(
                ...
                @{ label = 'OL';                            data = $table.ForEach({ $_.ol            / 1000 / 1000 / 1000 / 1000 }); hidden = $true }
